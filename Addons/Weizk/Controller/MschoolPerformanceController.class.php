...
class MschoolPerformanceController extends AddonsController{
    protected $model;
    protected $tableName = 'zk_mschool_performance';
    ...
    public function __construct() {
        ...
        parent::__construct ();
        $this->model = $this->getModel('ZkMschoolPerformance');
    ...
    private function import_performance_from_exl($file_id,$item_year) {
        ...
        $performance_model = D('zk_mschool_performance');
        if ($data['status']) {
            foreach  ($data['data'] as $key=>$row) {
                if($key < 4) {
                    ...
                    for($x = 1; $x<8; $x++){
                        $item['mschool_name'] = $row['school_name'];
