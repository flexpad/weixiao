  ...
  <div class="span9 page_message">
    <section id="contents">
      <ul class="tab-nav nav">
        <li class=""><a href="<?php echo U('lists');?>">课程列表</a></li>
        <li class=""><a href="<?php echo U('add');?>">编辑课程<b class="arrow fa fa-sort"></b></a></li>
        <li class="current"><a href="<?php echo U('scoreimport');?>">成绩导入<b class="arrow fa fa-sort"></b></a></li>
      </ul>
      <div class="tab-content"> 
        <!-- 表单 -->
        <form id="form" action="<?php echo U('scoreimport?model='.$model['id']);?>" method="post" class="form-horizontal">
            <div class="form-item cf">
                <label class="item-label"><span class="need_flag">*</span>课程（仅列出最近一年的课程，历史数据编辑可从列表项点击进入）<span class="check-tips"></span></label>
                <div class="controls">
                    <select name="course">
                        <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option><?php echo ($vo["id"]); ?>. <?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
              </div>
            <div class="form-item cf">
                ...
                <div class="controls">
                    <input type="datetime" name="classdate" class="text date" value="<?php echo ($field["classdate)"]); ?>" placeholder="请选择日期" />
                </div>
              </div>
            <div class="form-item cf">
            ...
            <div class="form-item cf">
                <label class="item-label"><span class="need_flag">*</span>描述<span class="check-tips">导入成绩数据说明，控制在100汉字以内</span></label>
                <div class="controls">
