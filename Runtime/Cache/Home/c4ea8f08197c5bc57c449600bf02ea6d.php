...
<div class="container">
    ...
    <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
        	<a href="<?php echo ($vo["url"]); ?>">
            	<?php if(!empty($vo["cover"])): ?><img src="<?php echo ($vo["coverurl"]); ?>"/><?php endif; ?>
                <h6><?php echo ($vo["name"]); ?></h6>
                <span class="colorless">&nbsp;&nbsp;课时数：<?php echo ($vo["length"]); ?> &nbsp;开课时间：<?php echo ($vo["sdate"]); ?></span>
            </a>
                    ...
                    if (json){
                        $.each(json, function(index, array){
                            if (array.cover)
                                ...
                                sstr = "";
                                str =  '<li><a href = \"' + array.url + '\">' + sstr + '<h6>' + array.name + '</h6>' + '<p>' + '课时数：' + array.length +'</p>'+'<span class="colorless">开课时间：'+array.sdate+ '</span></a></li>';
